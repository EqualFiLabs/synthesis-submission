...
> **Settlement mode note:** The `0xsettled-{agreementId}-{timestamp}` tx hashes in this file are generated by the local settlement webhook mock (see `SKILL.md` Step 4). They confirm the relayer's settlement pipeline executed end-to-end but are **not real on-chain transactions**. Real on-chain settlement is demonstrated in the relayer's Anvil integration tests and the Pure Financing demo (`PURE-FINANCING-TIMEWARP-OUTPUTS.md`), which produce actual Anvil tx hashes verified via `eth_getTransactionReceipt`.
